# Validation Checklist

**Gap ID:** [CH##-GAP-##]
**Gap Description:** [Brief description]
**Chapter:** [Number] — [Title]
**Validation Date:** [Date]

---

## Research Received

| Model  | File Attached | Approximate Length |
| ------ | ------------- | ------------------ |
| Claude | ☐ Yes / ☐ No  | [Pages/words]      |
| Gemini | ☐ Yes / ☐ No  | [Pages/words]      |

---

## Seven-Dimension Assessment

### 1. Coverage

**Question:** Did the research actually answer the question, or did it drift?

| Rating     | Description                                             |
| ---------- | ------------------------------------------------------- |
| ☐ Strong   | Directly addresses all aspects of the research question |
| ☐ Adequate | Addresses the core question, minor aspects untouched    |
| ☐ Partial  | Addresses some aspects but misses key elements          |
| ☐ Drift    | Went off-topic; question not meaningfully answered      |

**Notes:**
[Specific observations about coverage]

**Unaddressed Aspects:**
[List any parts of the question that weren't covered]

---

### 2. Depth

**Question:** Is there enough evidence for how the chapter will use it?

**Required Depth Level:** [From prompt: e.g., "2-3 paragraphs" or "cornerstone claim"]

| Rating         | Description                                    |
| -------------- | ---------------------------------------------- |
| ☐ Exceeds      | More evidence than needed; can select the best |
| ☐ Sufficient   | Meets the calibrated need                      |
| ☐ Thin         | Usable but minimal; could be stronger          |
| ☐ Insufficient | Not enough to support the chapter's use        |

**Notes:**
[Assessment of evidence quantity and quality]

**Evidence Count:**

- Case studies: [X]
- Statistics/data points: [X]
- Expert quotes: [X]
- Examples: [X]

---

### 3. Source Quality

**Question:** Are sources verifiable, authoritative, properly cited?

| Rating     | Description                                          |
| ---------- | ---------------------------------------------------- |
| ☐ Strong   | Primary sources, peer-reviewed, fully cited          |
| ☐ Adequate | Mix of quality; most properly cited                  |
| ☐ Mixed    | Some good sources but also questionable ones         |
| ☐ Weak     | Poor sources, missing citations, unverifiable claims |

**Primary Sources Found:** ☐ Yes / ☐ No
[List them:]

**Citation Completeness:**
| Source | Full Citation? | Verifiable? |
|--------|---------------|-------------|
| [Source 1] | ☐ Yes / ☐ No | ☐ Yes / ☐ No |
| [Source 2] | ☐ Yes / ☐ No | ☐ Yes / ☐ No |
| [Source 3] | ☐ Yes / ☐ No | ☐ Yes / ☐ No |

**Red Flags:**
[Note any concerning sources—unverified, biased, outdated, etc.]

---

### 4. Contradiction Check

**Question:** Did Claude and Gemini agree? Are there conflicts needing reconciliation?

| Rating            | Description                                    |
| ----------------- | ---------------------------------------------- |
| ☐ Agreement       | Both models aligned on key findings            |
| ☐ Complementary   | Different emphases but no conflicts            |
| ☐ Minor Conflicts | Small disagreements, easily reconciled         |
| ☐ Major Conflicts | Significant disagreements requiring resolution |

**Areas of Agreement:**
[What did both models confirm?]

**Areas of Disagreement:**
[Where did they differ? On what specifically?]

**Reconciliation Needed:**
☐ No — findings can be combined
☐ Yes — specify: [What needs resolving]

---

### 5. Thesis Tension

**Question:** Did anything challenge or complicate the book's thesis?

| Rating        | Description                        |
| ------------- | ---------------------------------- |
| ☐ Supports    | Research strengthens the thesis    |
| ☐ Neutral     | Neither supports nor challenges    |
| ☐ Complicates | Raises issues that need addressing |
| ☐ Challenges  | Serious tension with core argument |

**Tensions Identified:**
[Describe any findings that complicate or contradict the book's thesis]

**Severity:**
☐ Minor — can acknowledge and move on
☐ Moderate — needs explicit addressing in chapter
☐ Serious — may require thesis adjustment

**Author Decision Required:** ☐ Yes / ☐ No
[If yes, describe what needs deciding]

---

### 6. Usability

**Question:** Is this actually usable for drafting? Specific, quotable, concrete?

| Rating             | Description                                    |
| ------------------ | ---------------------------------------------- |
| ☐ Ready to Use     | Can draft with this immediately                |
| ☐ Needs Processing | Good material but needs organizing/condensing  |
| ☐ Partially Usable | Some good material, some unusable              |
| ☐ Not Usable       | Too vague, no specifics, can't draft from this |

**Usability Checklist:**

- ☐ Named examples (not generic)
- ☐ Specific numbers/dates
- ☐ Quotable statements with attribution
- ☐ Concrete enough to write from
- ☐ Organized/scannable

**Best Material:**
[Highlight the most usable findings]

**Unusable Material:**
[Note what can't be used and why]

---

### 7. Gap Spawning

**Question:** Did answering this reveal NEW gaps we didn't anticipate?

| Rating        | Description                             |
| ------------- | --------------------------------------- |
| ☐ None        | No new gaps emerged                     |
| ☐ Minor       | Small questions arose, not critical     |
| ☐ Significant | New important gaps identified           |
| ☐ Major       | Research revealed substantial new needs |

**New Gaps Identified:**

| New Gap       | Priority     | Add to Tracker? |
| ------------- | ------------ | --------------- |
| [Description] | P1 / P2 / P3 | ☐ Yes / ☐ No    |
| [Description] | P1 / P2 / P3 | ☐ Yes / ☐ No    |

---

## Overall Assessment

### Dimension Summary

| Dimension              | Rating   | Critical Issue? |
| ---------------------- | -------- | --------------- |
| 1. Coverage            | [Rating] | ☐ Yes / ☐ No    |
| 2. Depth               | [Rating] | ☐ Yes / ☐ No    |
| 3. Source Quality      | [Rating] | ☐ Yes / ☐ No    |
| 4. Contradiction Check | [Rating] | ☐ Yes / ☐ No    |
| 5. Thesis Tension      | [Rating] | ☐ Yes / ☐ No    |
| 6. Usability           | [Rating] | ☐ Yes / ☐ No    |
| 7. Gap Spawning        | [Rating] | ☐ Yes / ☐ No    |

---

## Verdict

### ☐ COMPLETE

Research satisfies all dimensions. Gap is filled.

**Strengths:**
[What was particularly good about this research]

---

### ☐ NEEDS MORE

Research fell short in specific areas.

**Deficiencies:**
[Exactly what's missing]

**Follow-up Required:**
[What the follow-up prompt should request]

**Follow-up Prompt Created:** ☐ Yes — File: [filename]

---

### ☐ PROBLEMATIC

Serious issue requiring author decision.

**Nature of Problem:**
[Describe the issue—thesis tension, contradictions, etc.]

**Decision Required:**
[What the author needs to decide]

**Options:**

1. [Option A and implications]
2. [Option B and implications]
3. [Option C and implications]

**Proceed After:** Decision is made and documented

---

## Tracker Updates

- [ ] Chapter Research Tracker updated
- [ ] Status changed to: [New status]
- [ ] Model coverage recorded
- [ ] Quality verdict recorded
- [ ] Notes added
- [ ] Gap-spawned items added (if any)

---

_Complete this checklist for every gap during validation._
