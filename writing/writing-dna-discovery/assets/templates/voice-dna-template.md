# Voice DNA: [Author Name]

## Mode: [Register - e.g., Blog, Fiction, Technical, Essay]

## Version: [X] | Created: [Date] | Last Updated: [Date]

---

## Quick Reference

**Core Temperature:** [e.g., "Warm but direct," "Cool and analytical," "Enthusiastic and conversational"]

**Sentence Signature:** [e.g., "Short punchy sentences with occasional long unwinders," "Long flowing sentences with multiple clauses," "Varied rhythm—unpredictable"]

**Distinctive Moves:**

- [Most recognizable pattern #1]
- [Most recognizable pattern #2]
- [Most recognizable pattern #3]

**Never Does:**

- [Key anti-pattern #1]
- [Key anti-pattern #2]

---

## Voice Profile

### Sentence Level

**Status:** [Not Assessed / Emerging / Developed / Strong]

[Analysis of sentence rhythm, length patterns, internal structure, emphasis placement]

**Patterns Identified:**

-
- **Examples from Their Writing:**

> "[Quote demonstrating pattern]"

---

### Punctuation Personality

**Status:** [Not Assessed / Emerging / Developed / Strong]

[Analysis of punctuation usage: em-dashes, semicolons, colons, parentheses, comma density, exclamation points, ellipses]

| Punctuation | Usage Pattern |
| ----------- | ------------- |
| Em-dashes   |               |
| Semicolons  |               |
| Parentheses |               |
| Commas      |               |
| Other       |               |

---

### Paragraph & Structure

**Status:** [Not Assessed / Emerging / Developed / Strong]

[Analysis of paragraph construction, length patterns, transitions, section organization]

**Paragraph Tendencies:**

-
- **Transition Style:**

- ***

### Word Choice & Vocabulary

**Status:** [Not Assessed / Emerging / Developed / Strong]

[Analysis of vocabulary level, Anglo-Saxon vs. Latinate preferences, range]

**Favorite Words/Phrases:**

-
-
- **Avoided Words/Phrases:**

-
-
- **Contraction Usage:** [Always / Often / Sometimes / Rarely / Never]

  **Jargon Comfort:** [Defines terms / Uses casually / Avoids]

  ***

### Tone & Attitude

**Status:** [Not Assessed / Emerging / Developed / Strong]

**Emotional Temperature:** [e.g., Warm / Cool / Neutral]

**Formality Level:** [e.g., Casual / Conversational / Professional / Formal]

**Confidence Style:** [e.g., Direct assertion / Hedged / Varies by topic]

**Notes:**

---

### Reader Relationship

**Status:** [Not Assessed / Emerging / Developed / Strong]

**First Person Presence:** [Heavy "I" usage / Moderate / Minimal / Avoids]

**Second Person Usage:** [Frequent "you" / Occasional / Rare / Never]

**Reader Assumptions:** [Assumes expertise / Assumes basics / Explains everything]

**Authority Stance:** [Expert / Guide / Fellow traveler / Curious explorer]

---

### Opening & Closing Moves

**Status:** [Not Assessed / Emerging / Developed / Strong]

**How They Begin Pieces:**
[e.g., In media res, provocative statement, question hook, scene-setting, anecdote]

**How They End Pieces:**
[e.g., Callback to opening, open question, definitive statement, quiet landing, call to action]

**Examples:**

> Opening: "[Quote]"
> Closing: "[Quote]"

---

### Humor Approach

**Status:** [Not Assessed / Emerging / Developed / Strong / Not Applicable]

**Humor Present:** [Yes / No / Occasionally]

**Type:** [Dry wit / Self-deprecation / Wordplay / Sarcasm / Absurdist / Other]

**Placement:** [Woven throughout / Strategic moments / Openings/closings / Asides]

---

### Signature Elements

**Status:** [Not Assessed / Emerging / Developed / Strong]

[Distinctive moves, pet phrases, characteristic tics—things unique to this writer]

**Distinctive Moves:**

-
- **Pet Phrases:**

-
- **Characteristic Tics:**

-
- ***

## Register-Specific Notes

[Include if relevant to the mode being captured]

### For Fiction

- **Narrative Distance:** [Close third / Distant third / First person intimate / etc.]
- **Dialogue Style:** [Tags / Beats / Dialect rendering]
- **Description Density:** [Sparse / Rich / Varies]
- **Interiority Access:** [Deep / Limited / External only]

### For Non-Fiction

- **Argument Style:** [Thesis-first / Build-up / Exploratory]
- **Evidence Handling:** [Data-driven / Anecdote-heavy / Mixed]
- **Counterargument Approach:** [Addresses directly / Ignores / Concedes then pivots]

### For Blog/Casual

- **Hook Pattern:** [Question / Bold claim / Story / Direct address]
- **Personal Disclosure:** [High / Moderate / Low]
- **CTA Style:** [Direct / Soft / Absent]

---

## Exemplar Passages

[Annotated excerpts that show patterns in action]

> "[Quote from their writing—a paragraph or two that exemplifies their voice]"
>
> — _Demonstrates: [List patterns this shows: sentence rhythm, tone, signature move, etc.]_

> "[Another representative quote]"
>
> — _Demonstrates: [What this shows]_

> "[A third quote, ideally showing a different aspect]"
>
> — _Demonstrates: [What this shows]_

---

## Anti-Patterns

[What they never do, with reasoning]

| Pattern to Avoid                    | Why It's Wrong for Them                       |
| ----------------------------------- | --------------------------------------------- |
| [e.g., Em-dash chains]              | [e.g., They use em-dashes sparingly for punch |
| [e.g., "It's important to note..."] | [e.g., Too hedging; they assert directly]     |
| [e.g., Passive voice heavy]         | [e.g., They prefer active, direct statements] |
|                                     |                                               |

### AI Patterns to Suppress

[Check those that apply to this writer]

- [ ] Em-dash overuse
- [ ] "It's important to note/remember"
- [ ] "In conclusion" / "In summary"
- [ ] Significance puffery ("stands as a testament," "plays a vital role")
- [ ] "Rich tapestry," "nestled," "in the heart of"
- [ ] "Despite challenges..." formula
- [ ] "Delve into," "navigate the complexities"
- [ ] Rule of three overuse
- [ ] Elegant variation (excessive synonym-swapping)
- [ ] "This [noun] serves as..."
- [ ] "At its core"
- [ ] Hedging with "various" and "numerous"
- [ ] Overly balanced sentence structures
- [ ] [Other identified patterns]

---

## Ghost Writer Briefing

[This section is specifically formatted for the downstream ghost writer skill]

### Voice Essence

[2-3 sentence distillation of their voice—the north star for the ghost writer]

### Do This

- [Specific actionable instruction #1]
- [Specific actionable instruction #2]
- [Specific actionable instruction #3]
- [Specific actionable instruction #4]
- [Specific actionable instruction #5]

### Don't Do This

- [Specific thing to avoid #1]
- [Specific thing to avoid #2]
- [Specific thing to avoid #3]
- [Specific thing to avoid #4]
- [Specific thing to avoid #5]

### When Uncertain

[Decision rules for ambiguous situations]

- [e.g., "Default to shorter rather than longer"]
- [e.g., "When in doubt, cut the adjective"]
- [e.g., "Favor concrete over abstract"]

### Sentence-Level Guidance

- **Average length:** [X words, vary between Y-Z]
- **Opening patterns:** [How to start sentences]
- **Closing patterns:** [Where to place emphasis]

### Structural Guidance

- **Paragraph length:** [Tendency]
- **Transitions:** [How to move between ideas]
- **Piece opening:** [How to begin]
- **Piece closing:** [How to end]

---

## Profile Metadata

**Readiness Level:** [Minimum Viable / Solid / Strong]

**Confidence Assessment:**

| Dimension             | Confidence |
| --------------------- | ---------- |
| Sentence Level        |            |
| Punctuation           |            |
| Word Choice           |            |
| Tone                  |            |
| Reader Relationship   |            |
| Signature Elements    |            |
| Anti-Patterns         |            |
| Ghost Writer Briefing |            |

**Sample Base:**

- [Sample 1: Description, length, date]
- [Sample 2: Description, length, date]
- [Sample 3: etc.]

**Dimensions Needing Depth:**

-
- **Open Questions:**

-
- ***

## Session History

| Date | Version | Focus | Key Updates |
| ---- | ------- | ----- | ----------- |
|      |         |       |             |
|      |         |       |             |
|      |         |       |             |

---

## Overnight Question

[Question to sit with before next session]

> "[e.g., Notice a piece of writing—yours or someone else's—that makes you think 'that's exactly how I would/wouldn't say that.' Bring it back next time.]"

---

## Notes

[Any additional observations, context, or reminders for future sessions]
